...
def init_squeezer():
	...
	yp=hstack((zeros(7,),array([                    # 7 zeros = dotted variables
...
def squeezer1(t, y):
	...
	gqq=zeros((6,))
	gqq[0] = -rr*cobe*y[0]**2 + d*cobeth*(y[0] + y[1])**2 + ss*siga*y[2]**2
	gqq[1] = -rr*sibe*y[0]**2 + d*sibeth*(y[0] + y[1])**2 - ss*coga*y[2]**2
	gqq[2] = -rr*cobe*y[0]**2 + d*cobeth*(y[0] + y[1])**2 + e*siphde*(y[3] + y[4])**2 + zt*code*y[4]**2
	gqq[3] = -rr*sibe*y[0]**2 + d*sibeth*(y[0] + y[1])**2 - e*cophde*(y[3] + y[4])**2 + zt*side*y[4]**2
	gqq[4] = -rr*cobe*y[0]**2 + d*cobeth*(y[0] + y[1])**2 + zf*coomep*(y[5] + y[6])**2 + u*siep*y[6]**2
	gqq[5] = -rr*sibe*y[0]**2 + d*sibeth*(y[0] + y[1])**2 + zf*siomep*(y[5] + y[6])**2 - u*coep*y[6]**2

	#     Construction of the residual
	A = np.bmat( [ [m, gp.T], [gp, zeros([6, 6])] ] )	
	b = np.bmat( [ [ff[0:7].T], [-gqq.T]    ]).T
	w_lamb = solve(A, b)
	w = w_lamb[0:7]
	lamb = w_lamb[7:]
